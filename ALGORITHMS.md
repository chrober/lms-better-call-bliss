...
Better Call Bliss turns a saved playlist or player-queue snapshot into a smoother listening experience. You decide whether existing songs may move, whether new songs may be added, and how much freedom the optimizer has. Playlist and queue-editor jobs preview the result, and nothing is changed until you accept it. The three track- and album-context **Bliss me there...** shortcuts are intentionally different: they run in the background and apply a successful route automatically.  
...
| [Add spacing tracks as needed](#add-spacing-tracks-as-needed) | Working | The source contains repeated artists or albums too close together for the selected repeat windows. | Starts with the smallest repeat-safe final size and grows only if that size cannot be routed within the selected repeat windows. It does not add optional bridges for merely awkward transitions. |
...
The Better Call Bliss plugin resolves Lyrion tracks, reads per-job options, freezes the LMS-local candidate inventory and current play counts when needed, and optionally asks LastMix for Last.fm track and artist relationships. It resolves those Last.fm results to explicit candidates in that frozen local inventory before launching the native [bliss-playlist-optimizer](https://github.com/chrober/bliss-playlist-optimizer), which performs provider-neutral scoring and bounded search. Only the plugin writes playlists or player queues. Editor jobs require explicit acceptance; the **Bliss me there...** shortcuts automatically append, replace upcoming tracks, or insert an excursion only after the background route and matching live-anchor checks succeed.
...
Use any of the three commands from a local track's or album's context menu when a player already has something in its queue and you want to arrive at the selected music smoothly. Each command closes the context menu and starts a background job with the saved **Bliss me there...** defaults. None opens the Extras page or requires a separate Accept button. The job remains visible under **Running and recent previews** for status or error review.  
...
- **Bliss me there... and back again!** fixes the current song as the start, the selected destination, and the first upcoming song as the rejoin. It inserts the outward route, destination, and return route immediately after the current song without removing the captured upcoming queue.  
...
A selected track is one fixed destination member. A selected album is a fixed destination block containing every track in canonical disc and track order. Better Call Bliss never reorders that block and never inserts songs between its tracks. It searches only the approach to the album's first track and, for **and back again**, the return from the album's last track. Album-internal transitions are part of the user's chosen recording and are not judged as bridge-search failures. If any album member is not a local audio track or cannot be resolved through Bliss, the action fails instead of silently playing an incomplete album.  

The selected track or album is the fixed destination for the two one-way commands and a fixed visit for **and back again**. The latter also locks the first upcoming song as its rejoin anchor. A bounded local queue prefix ending at the chosen start is captured separately as immutable context: it can provide Adaptive acoustic context, repeat context, and optional Last.fm evidence, but its earlier tracks are not route members, acoustic path endpoints, or output tracks. For the queue-end command this prefix may include songs that are still upcoming. For both current-song commands, later queue entries are excluded from backward-looking context. Repeats already present in captured history are tolerated.  

**Choose automatically** first measures the unbridged route boundaries under the configured BlissMixer strategy: start-to-destination entrance for a one-way route, or start-to-destination entrance and destination exit-to-rejoin for an excursion. With a track, entrance and exit are the same song; with an album they are its first and last tracks. Static uses the current Static feature weights. Adaptive builds one contextual matrix from the bounded captured context plus the route start, applies the configured learned-matrix blend, and uses the same learned/Static fallback rules as BlissMixer. **Normal** caution may use the unbridged route when the minimum is zero and its governing view meets the target. **Cautious** additionally compares available Static and learned-only measurements and starts a bridge search when a secondary view disagrees with the governing view by at least 25 percentile points. It then uses the worst available whole-boundary verdict for acceptance and ranking, so a route cannot win merely because one model overlooks an abrupt change. A dedicated layered search covers every permitted total count from **Minimum intermediate tracks** through **Maximum intermediate tracks**. For an excursion, that is one budget shared by the approach and return boundaries rather than a separate count per boundary. Automatic chooses the shortest complete route that meets the adjacent target across the applicable model view or views. If no path reaches the target and zero intermediates are permitted, every best-effort bridge path is compared with the unbridged boundaries: Better Call Bliss inserts bridges only when they improve the cautious worst-model result by at least one percentile point. Otherwise it keeps the unbridged route and explicitly reports that no beneficial bridge was found. A positive minimum remains an explicit request for intermediates. **Use an exact count** runs the same adjacent path search for precisely the requested total number of intermediates and remains all-or-nothing.  

Candidate discovery is frozen before the layered path search begins. For a one-way route, the acoustic prefilter compares every eligible candidate C with both endpoints: `start -> C` and `C -> destination entrance`. For **and back again**, it creates that same kind of balanced shortlist independently for `start -> destination entrance` and `destination exit -> rejoin`. Optional Last.fm matches from each boundary's anchors and a bounded suffix of captured context reserve some places in the relevant shortlist. The optimizer searches complete outward paths, then evaluates each retained outward result together with return paths under the remaining shared budget. This makes route membership and repeat-window checks span both boundaries. A chosen intermediate affects the next path edge and repeat checks, but it does not expand either pool or trigger another library-wide discovery round. Every album member is excluded from bridge candidates.  

For every final adjacent edge on the generated approach or return boundary, the optimizer ranks its fixed-matrix distance against `A -> every current LMS-local analyzed track` under the same matrix. That whole local reference population turns the raw edge distance into the destination-routing percentile described under [Understanding transition-quality percentiles](#understanding-transition-quality-percentiles); it is not another candidate pool. The result identifies the governing Adaptive-context or Static role and hash, records the effective Adaptive algorithm, seed identities, configured learned share, fallback reason, available direct-boundary model verdicts, and every generated boundary edge, route sum, raw bottleneck, and worst adjacent percentile. Album-internal and earlier listening-history edges are not included in this destination-path quality result. Under Cautious it reports and applies distinct Static and learned-only whole-boundary measurements as well.  
...
flowchart LR
    ...
    Q["Chosen queue prefix<br/>... -> H -> A (start)"] --> F["Find A -> destination entrance"]
    D["Selected track B<br/>or album B1 -> ... -> Bn"] --> F
    E --> F
    M["Optional Last.fm<br/>ranking support only"] -.-> F
    F --> R["One-way result<br/>... -> A -> +X -> B<br/>or B1 -> ... -> Bn"]
    U["For and back again:<br/>first upcoming track U"] --> G["Also find destination exit -> U<br/>within the shared budget"]
    F --> G
    E --> G
    G --> T["Inserted excursion<br/>A -> +X -> destination -> +Y -> U -> ..."]
~~~
...
The acoustic prefilter reduces a large library without assuming that one song must resemble both endpoints perfectly. For each locked boundary, it reserves candidates closest to that boundary's left anchor, candidates closest to its right anchor, and candidates with the best balanced pair of distances. A one-way track route has the single boundary `A -> B`; an album route uses `A -> B1`. **And back again** also has `B -> U` for a track or `Bn -> U` for an album, so it prepares two shortlists under the same frozen matrix. Optional Last.fm-supported tracks can reserve some additional shortlist positions, but their Bliss edge distances still govern the path.  
...
3. Repeat-window violations and paths that cannot approach the destination boundary competitively are discarded. Search effort controls how many next choices per partial path and how many partial paths survive each layer.
4. When the requested layer is complete, the destination entrance is appended and the real adjacent edges of every retained complete path are measured.
...
For **and back again**, the outward candidates are not finalized independently and pasted onto an unrelated return route. The optimizer retains bounded outward alternatives for each possible outward bridge count, carries each complete outward route—including its chosen tracks and repeat state—into the return-boundary search, and gives that boundary only the unused portion of the total budget. It then ranks the complete `A -> ... -> destination -> ... -> U` alternatives by the worst adjacent jump across both boundaries and by their total distance. This is a bounded joint search, so search effort still controls breadth, but route uniqueness, repeat windows, quality reporting, and the bridge budget cover the excursion as one result. An album's internal sequence remains an untouched block inside every alternative.  
...
flowchart TD
    ...
    E --> V
    V -- Queue end valid --> O["Append route suffix<br/>... -> A -> +X -> destination"]
    V -- Current song valid --> N["Keep A; replace upcoming queue<br/>... -> A -> +X -> destination"]
    V -- Current and rejoin valid --> R["Insert excursion before U<br/>... -> A -> +X -> destination -> +Y -> U -> ..."]
...
For **and back again**, the selected track or an album member may also occur farther ahead in the preserved queue. The excursion still inserts the complete selected destination and leaves later occurrences untouched, because preserving the existing upcoming queue is the defining behavior of this command. Selecting a track that is the first upcoming song, or an album containing it, is rejected: that song is already the locked rejoin point, so it cannot simultaneously belong to a meaningful excursion away from and back to the queue.  
...
Better Call Bliss calculates the theoretical minimum final size required by the repeated source artists and albums. It then asks the native optimizer to try that smallest repeat-safe target first. If that exact target cannot be routed while keeping every repeat-window proof true, the optimizer grows the target one track at a time until it finds the smallest routeable result or reaches the configured **Maximum additional tracks** budget.
...
| Maximum additional tracks | 0-100; plugin default | Upper bound for dynamic spacing additions. If the repeat windows need more room than this, the preview fails before launching the optimizer and reports the minimum required final size. If the minimum is theoretically possible but not routeable, the optimizer may use more additions up to this limit. |
...
The base strategy and settings come from the original [BlissMixer](https://github.com/CDrummond/lms-blissmixer). Better Call Bliss requires a compatible original BlissMixer installation and reuses the shared native Bliss scoring core so both applications interpret the 23 Bliss audio features consistently. Optional learned personalization comes from [BlissMixerLab](https://github.com/chrober/lms-blissmixer-lab), which owns `learned_matrix.json` and its learned-blend preference. BlissMixer's Last.fm artist probability and BlissMixerLab's Last.fm track guidance apply to their immediate-mix weighted sampling; Better Call Bliss therefore keeps separate defaults for its bounded route and playlist evidence.
...
For Better Call Bliss and the shared Adaptive algorithm, **yes**. BlissMixerLab optionally produces the learned matrix through its similarity survey and training process:
