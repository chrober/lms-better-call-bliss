...
This document describes the current 0.14.0 implementation. **Working** means the choice is available in the Lyrion job editor. **Planned** means it is shown but cannot yet be selected.
...
| Generation seed | 0–4,294,967,295; generated by default | Reproduces the optimized source route when source order may move. |
| Use Last.fm guidance | Inherited; optional | Enables failure-tolerant track and artist evidence through LastMix. |
| Similar-track guidance | 0–100%; default 75 | Bounded support from recording relationships for each possible insertion. |
| Similar-artist guidance | 0–100%; default 75 | Bounded support from local artist relationships and the original-collection fallback. |
