...
The selected mode chooses the shortest route within its configured minimum and maximum that meets its neighboring-transition target, or the smoothest bounded best effort. Exact uses the same path objective for its configured count. Fast, Balanced, and Thorough effort profiles trade search breadth for runtime; Fast is the deliberately bounded default.  
