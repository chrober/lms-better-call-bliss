...
This directory contains the installable `0.17.1` Better Call Bliss plugin package. Releases provide separate Linux, macOS, and Windows archives; each contains only the optimizer binaries usable on that target family. It contributes **Better Call Bliss** under Extras and exposes playlist/track context shortcuts for supported controllers such as Material Skin. Lyrion's Applications/OPML adapter cannot expose a portable rich multi-field form, so the job editor uses the same classic-web mechanism as Virtual Library Creator.
...
The connected track paths start a saved-default native destination route in the background without opening the Extras page. In menu order, **Bliss me there...** starts at the current song, preserves it and playback, and replaces only the later queue entries with the route suffix. **Bliss me there... and back again!** treats the selected track as a required waypoint, inserts the outward route, waypoint, and return route after the current song, and rejoins the unchanged upcoming queue at its first track. **Bliss me there... when we're through!** starts at the queue end and appends the generated route suffix. The round trip's one configured intermediate-track budget is shared across both route legs. Their requests separate repeat-tolerant immutable listening history from unique route anchors.  
Automatic uses a dedicated fixed-matrix adjacent path search and offers Normal or Cautious direct-transition handling. Static uses the current BlissMixer feature weights. Adaptive builds a per-run matrix from bounded analyzed history ending at the chosen route start, applies BlissMixerExt's configured learned blend when its optional learned matrix is available, and otherwise follows the variance/Static fallback rules. The matrix remains fixed during that route search.
...
BlissMixer supplies the base strategy, context, feature-weight, genre, and repeat defaults. When installed, BlissMixerExt supplies the optional learned matrix, learned-blend default, and durable Last.fm similar-track guidance preference. Better Call Bliss consumes that current track-guidance value instead of storing a duplicate setting. Every job may override the available values, route-search restart count, strategy-neutral Variation, and optional Last.fm similar-track and similar-artist guidance.
Similar-track guidance falls back to 25% when BlissMixerExt 0.3.0 or newer is unavailable; the Better Call Bliss similar-artist default is 25%. Both only rerank local, Bliss-qualified candidates. Setting an artist or album window to zero disables that constraint, allowing single-artist or single-album collections to be optimized.

Every feature that can add music also snapshots and enforces BlissMixer's current genre settings. Configured genre groups and glob patterns, Match all genres, Use track genre, and the December-aware Christmas exclusion are applied in the optimizer's single shared candidate-library boundary. They constrain generated additions only; existing playlist tracks, listening history, selected destinations, mandatory waypoints, and queue-rejoin anchors remain valid user input. Completed previews and information logs report the genre and Christmas rejection counts.  

Every addition job also freezes a per-job Lyrion **Candidate library**. The Extras dropdown initially follows Material Skin's active virtual library when available, then the library assigned to the active player, and otherwise uses All tracks. Only newly generated tracks must belong to that `library_track` membership; immutable source, history, destination, waypoint, and queue-rejoin anchors remain valid outside it. The membership checksum invalidates cached candidate inventories after a virtual-library rebuild.  

The plugin delegates acoustic scoring, route search, bridge selection, and fixed-source extension membership selection to an installed `bliss-playlist-optimizer` binary. Better Call Bliss owns Lyrion identity capture, optional LastMix/Last.fm evidence collection, Preview polling, result validation, and playlist or player-queue persistence, including replacing only the upcoming part of an active queue. The original BlissMixer is required for `bliss.db` and its base settings. BlissMixerExt is optional and contributes `learned_matrix.json`, its learned-blend preference, and its Last.fm similar-track guidance preference; without it, Adaptive falls back to variance plus Static BlissMixer weights for one-track contexts and track guidance uses the 25% fallback.
