...
This directory contains the installable `0.17.2` Better Call Bliss plugin package. Releases provide separate Linux, macOS, and Windows archives; each contains only the optimizer binaries usable on that target family. It contributes **Better Call Bliss** under Extras and exposes playlist/track context shortcuts for supported controllers such as Material Skin. Lyrion's Applications/OPML adapter cannot expose a portable rich multi-field form, so the job editor uses the same classic-web mechanism as Virtual Library Creator.
...
BlissMixer supplies the base strategy, context, feature-weight, genre, and repeat defaults. When installed, BlissMixerExt supplies the optional learned matrix and learned-blend default. Every job may override the available values, route-search restart count, strategy-neutral Variation, and optional Last.fm similar-track and similar-artist guidance.
Both Last.fm guidance controls have independent Better Call Bliss defaults of 25%. They are deliberately separate from BlissMixer's artist-selection target and BlissMixerExt's immediate-mix track weighting because Better Call Bliss uses them as bounded evidence for playlist and route ranking. Both only rerank local, Bliss-qualified candidates. Setting an artist or album window to zero disables that constraint, allowing single-artist or single-album collections to be optimized.
