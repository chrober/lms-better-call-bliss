...
BlissMixer supplies the base strategy, context, feature-weight, genre, and repeat defaults. When installed, BlissMixerLab supplies the optional learned matrix and learned-blend default. Every job may override the available values, route-search restart count, strategy-neutral Variation, and optional Last.fm similar-track and similar-artist guidance.
Both Last.fm guidance controls have independent Better Call Bliss defaults of 25%. They are deliberately separate from BlissMixer's artist-selection target and BlissMixerLab's immediate-mix track weighting because Better Call Bliss uses them as bounded evidence for playlist and route ranking. Both only rerank local, Bliss-qualified candidates. Setting an artist or album window to zero disables that constraint, allowing single-artist or single-album collections to be optimized.
