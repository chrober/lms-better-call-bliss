...
This directory contains the installable `0.19.4` Better Call Bliss plugin package. Releases provide separate Linux, macOS, and Windows archives; each contains the optimizer and trusted guidance-provider binaries usable on that target family. It contributes **Better Call Bliss** under Extras and exposes playlist, track, and album context shortcuts for supported controllers such as Material Skin. Lyrion's Applications/OPML adapter cannot expose a portable rich multi-field form, so the job editor uses the same classic-web mechanism as Virtual Library Creator.
...
Use Lyrion's `plugin.bettercallbliss` log category to inspect a run. Information
level records readable sources, effective settings, Last.fm health and selected
evidence, final routes, quality, timing, and verified output. Debug adds artifact
paths, LMS identities/URLs, native stage timings, raw evidence for selected
additions, and per-leg governing plus secondary acoustic-model measurements.
Under Cautious automatic routing every distinct available acoustic view constrains
selection; under Normal the secondary views remain diagnostic.

The connected saved-playlist path selects a real saved playlist and starts a native Preview using the selected addition purpose: no additions, difficult-transition improvements, or **Extend playlist** (exact additions, final track count, or double count). Difficult-transition improvements use each local gap as their similarity base. Extend playlist selects new members against the complete original source set as a fixed reference, so **Reach a final track count** also covers the former seed-list growth workflow.  
It can route the complete source-plus-addition set freely or preserve the source-track order and place additions around those anchors. The connected player-queue path snapshots the full queue, the current-plus-upcoming segment, or only upcoming tracks and optimizes that immutable snapshot.  
The connected track and album paths start a saved-default native destination route in the background without opening the Extras page. An album is an immutable destination block: all of its tracks play in canonical disc and track order, while generated bridges are searched only before its first track and, for the round trip, after its last track. An album containing a non-local, non-audio, or unresolved Bliss member is rejected rather than silently truncated. In menu order, **Bliss me there...** starts at the current song, preserves it and playback, and replaces only the later queue entries with the route suffix. **Bliss me there... and back again!** inserts the outward route, complete destination, and return route after the current song, then rejoins the unchanged upcoming queue at its first track. **Bliss me there... when we're through!** starts at the queue end and appends the generated route suffix. The round trip's one configured intermediate-track budget is shared across both route boundaries. Their requests separate repeat-tolerant immutable listening history from unique route anchors.  
Automatic uses a dedicated fixed-matrix adjacent path search and offers Normal or Cautious direct-transition handling. Static uses the current BlissMixer feature weights. Adaptive builds a per-run matrix from bounded analyzed history ending at the chosen route start, applies BlissMixerLab's configured learned blend when its optional learned matrix is available, and otherwise follows the variance/Static fallback rules. The matrix remains fixed during that route search.
Cautious searches when the governing view strongly disagrees with a distinct available Static or learned-only view and judges complete paths across every available view; Normal uses the configured governing view alone.  
The selected mode chooses the shortest route within its configured minimum and maximum that meets its neighboring-transition target. If Automatic cannot meet that target and zero intermediates are allowed, a best-effort bridge is used only when it meaningfully improves the cautious direct result; otherwise the direct destination is retained and reported. Exact uses the same path objective for its configured count. Fast, Balanced, and Thorough effort profiles trade search breadth for runtime; Fast is the deliberately bounded default.  
A successful job automatically applies only the route body after checking that its captured anchors are still live. The queue-end command appends; the now-playing command replaces only the upcoming queue; the round-trip command inserts its excursion before the unchanged upcoming queue. Extend playlist keeps the full source playlist as its immutable relevance anchor.  
BlissMixer supplies the base strategy, context, feature-weight, genre, and repeat defaults. When installed, BlissMixerLab supplies the optional learned matrix and learned-blend default. Every job may override the available values, route-search restart count, strategy-neutral Variation, optional Last.fm similar-track and similar-artist target shares, and local play-count, last-played, and library-age influences.
Both Last.fm target-share controls have independent Better Call Bliss defaults of 25%. They remain separate preferences because Better Call Bliss plans complete playlists and routes, but use the same user-facing idea as BlissMixer's artist target: each is a best-effort share of added tracks supported by the indicated Last.fm channel. They guide only local, repeat-safe, Bliss-qualified candidates. Setting an artist or album window to zero disables that constraint, allowing single-artist or single-album collections to be optimized.

Every feature that can add music also snapshots and enforces BlissMixer's current genre settings. Configured genre groups and glob patterns, Match all genres, Use track genre, and the December-aware Christmas exclusion are applied in the optimizer's single shared candidate-library boundary. They constrain generated additions only; existing playlist tracks, listening history, selected destination tracks or albums, and queue-rejoin anchors remain valid user input. Completed previews and information logs report the genre and Christmas rejection counts.  

Every addition job also freezes a per-job Lyrion **Candidate library**. The Extras dropdown initially follows Material Skin's active virtual library when available, then the library assigned to the active player, and otherwise uses All tracks. Only newly generated tracks must belong to that `library_track` membership; immutable source, history, destination, and queue-rejoin anchors remain valid outside it. The membership checksum invalidates cached candidate inventories after a virtual-library rebuild.  

The plugin delegates acoustic scoring, route search, bridge selection, and fixed-source extension membership selection to an installed `bliss-playlist-optimizer` binary. Better Call Bliss owns Lyrion identity capture, optional LastMix/Last.fm evidence collection, the immutable Last.fm artifact, trusted provider executable paths, Preview polling, result validation, and playlist or player-queue persistence, including replacing only the upcoming part of an active queue. The optimizer invokes the bundled providers over its versioned local SPI only for Bliss-qualified shortlists. Last.fm remains artifact-backed and network-free in the native path. The local library-signals provider opens Lyrion's trusted `persist.db` read-only once, builds compact play-count, last-played, and library-age percentile distributions from the frozen eligible identity population, then queries only bounded Bliss-qualified batches while scoring. Providers may boost or de-boost ranking but can never admit a non-Bliss candidate or override hard constraints. The original BlissMixer is required for `bliss.db` and its base settings. BlissMixerLab is optional and contributes `learned_matrix.json` plus its learned-blend preference; without it, Adaptive falls back to variance plus Static BlissMixer weights for one-track contexts.
...
Every addition job first snapshots current local LMS track identities, intersects them with the selected Candidate library and usable `TracksV2` rows, and freezes the result. The checksum-protected allowlist is bound to the exact `bliss.db` file identity and virtual-library membership and applied natively before candidate shortlisting or scoring. Unmatched Bliss rows remain excluded even when their acoustic score would otherwise win. A persistent ledger at `<LMS cache>/bettercallbliss/non-lms-bliss-rows.json` records their paths, metadata, reasons, first/last-seen times, observation counts, and resolved/current state for review. Tracks outside the selected virtual library are reported separately and are not audit defects. The Extras page keeps the stale-row audit out of the normal workflow and shows it only when the current inventory has genuinely unmatched rows. Existing files are not automatically eligible: when a Bliss row differs from a unique LMS identity only by filename case, the audit records `filename_case_differs_from_lms_catalog` and the related LMS identity, preserving exact membership and preventing duplicate case-variant candidates.
