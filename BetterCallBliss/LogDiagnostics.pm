...
sub start_info_lines {
    ...
    my @lines = (
        ...
        sprintf(
            'Mixing strategy: %s; adaptive context %d; BlissMixerLab learned blend %d%%; learned matrix %s.',
...
sub result_info_lines {
    ...
    if (%$provenance) {
        push @lines, sprintf(
            'Scoring provenance: context %s; seeds %s; configured %s with BlissMixerLab learned share %d%%; effective base share %d%%; learned matrix %s.',
