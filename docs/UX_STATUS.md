...
| Track context > Bliss me there... | Working | Starts a destination route in the background using saved defaults, without opening Extras or requiring an Accept button. Recent queue entries are repeat-tolerant immutable history, separate from the unique tail/destination route members. Automatic offers Normal or Cautious bridge handling; Cautious also searches on strong learned/Static disagreement and judges candidate paths across both available views. It chooses the shortest route meeting its quality target or the smoothest repeat-safe bounded best effort. Exact requires its configured count. A successful job automatically appends only the suffix after a live-tail check; a stale or structurally infeasible exact job appends nothing and remains reviewable in recent previews. |
