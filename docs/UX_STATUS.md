...
| Optimize source order | Working for no additions, difficult-transition improvements, Extend playlist, and seed growth | Original tracks may move. Seed growth routes the complete final membership when this policy is selected. |
| Preserve source order | Working for difficult-transition improvements, Extend playlist, and seed growth | Every original track remains an immutable anchor in its input order. Difficult-transition improvements inspect source gaps. Extend playlist and seed growth keep the source order as anchors and place the selected additions around them when repeat windows can be satisfied. |
| Additional-track purpose | Working | The visible editor asks for the purpose first: no additions, improve difficult transitions, Extend playlist, or grow from these seeds. Extend playlist maps to the native `seed_growth` request after calculating the requested final size. Internal strict gap-bridge modes remain available only for specialized paths such as the first Bliss me there slice. |
...
| Extend playlist | Working | Shows a second amount selector for Add exactly N tracks, Reach final track count, or Double track count. The selected amount is honored through grow-style membership selection rather than being capped by internal source gaps. |
...
| Reach target track count | Working under Extend playlist | User enters the desired final track count. The plugin derives `N = T - S` and runs grow-style membership selection. |
| Grow from these seeds | Working | Keeps all `S` source tracks exactly once, uses their complete immutable set as the Adaptive relevance reference, selects exactly `T - S` LMS-local analyzed additions under repeat-window capacity, and either routes the complete `T`-track membership freely or preserves the source-track order while placing additions around those anchors. The default target is 25. |
...
| Double track count | Working under Extend playlist | Adds exactly `S` local tracks, yielding `2S` tracks, through grow-style membership selection. |
| Double duration | Not connected yet | Duration-based doubling remains future work. |
| Numeric editor for N/T | Working for Extend playlist and seed-growth targets | Exact-addition input is validated as 1-100 and converted to final size `S + N`. Target track count is validated as 3-500 and must exceed `S`. Double count is disabled when `2S` would exceed the 500-track target limit. |
...
| Additions and reasons | Working for difficult-transition improvements, Extend playlist, and seed growth | Gap-improvement mode labels every added bridge with its endpoints, direct-gap percentile, and evidence tier/pool. Extend playlist and seed growth label each addition with its distance from the fixed full-source Adaptive reference and report source/addition/final counts. |
