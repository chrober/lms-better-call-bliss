...
model and the exact boundary of the current `0.19.0` / `extras-job-editor-v23` UX shell. The shell is
...
| Optimize source order | Working for no additions, difficult-transition improvements, spacing-track repair, and Extend playlist | Original tracks may move. Spacing-track repair and Extend playlist route the complete final membership when this policy is selected. |
| Preserve source order | Working for difficult-transition improvements, spacing-track repair, and Extend playlist | Every original track remains an immutable anchor in its input order. Difficult-transition improvements inspect source gaps. Spacing-track repair and Extend playlist keep the source order as anchors and place the selected additions around them when repeat windows can be satisfied. |
| Additional-track purpose | Working | The visible editor asks for the purpose first: no additions, improve difficult transitions, add spacing tracks as needed, or Extend playlist. Spacing-track repair and Extend playlist map to the native fixed-source extension request; spacing repair passes a minimum target plus maximum budget, while Extend playlist keeps the chosen target exact. The dedicated destination-route mode is used only by Bliss me there; general playlist additions continue to use their own listener-facing purposes. |
...
| Improve difficult transitions | Working | Formerly shown as Add automatically. With optimized order it reorders the source before examining gaps; with preserved order it examines only the original gaps. It adds up to the per-job budget where the contextual trigger, acoustic-improvement, uniqueness, and repeat gates pass, and may add zero. Before candidate capture or Last.fm work, preserved order checks its immutable source anchors against the selected artist and album windows. A conflict fails immediately, names the repeated artist or album and source positions, and directs the listener to Add spacing tracks as needed or Optimize source order. |
| Add spacing tracks as needed | Working | Uses the source artists and albums to calculate the minimum repeat-safe final size, then lets the optimizer try that size first and grow only as far as needed within the per-job maximum-additions budget. It does not use the bridge-trigger percentile or inspect individual gaps for optional transition bridges. |
| Extend playlist | Working | Shows a second amount selector for Add exactly N tracks, Reach final track count, or Double track count. The selected amount is honored through fixed-source membership selection rather than being capped by internal source gaps. If the selected target is too small for the artist or album repeat windows, the preview fails before optimizer launch and reports the minimum repeat-safe final size. |
...
| Learned-matrix blend | Working when BlissMixerLab has a matrix | Initialized from BlissMixerLab and validated as 0-100. The control is read-only at an effective 0 when the optional extension or its matrix is unavailable. With two or more context tracks, 0 means pure variance weighting; one-track contexts then use Static BlissMixer weights. |
...
| Additions and reasons | Working for difficult-transition improvements, spacing-track repair, and Extend playlist | Gap-improvement mode labels every added bridge with its endpoints, direct-gap percentile, and evidence tier/pool. Spacing-track repair and Extend playlist label each addition with its distance from the fixed full-source Adaptive reference and report source/addition/final counts. |
...
Spacing-track repair and Extend playlist additionally require an exact target match, unique final membership, every source ID exactly once, the expected resolved additions for the calculated or chosen final size, and selection details for every added Bliss row. The native proof block must also affirm local-inventory membership and artist/album/track repeat compliance before the plugin accepts the result. Added tracks never enter the relevance anchor; they affect only complete-membership routing.

Running results refresh automatically and completed/failed optimization and accept actions are displayed in prominent banners with stable error codes. The Extras page shows immediate client-side feedback as soon as **Run read-only preview** is pressed, covering synchronous source capture and candidate-inventory preparation before a pollable job id exists. After a job starts, every polling, result, and accept-action page rebuilds the editor from that job's normalized options. Inactive numeric controls remain submitted as read-only values, so failures and successful review pages retain the exact settings for adjustment and rerun. After a successful preview, **Accept this preview** lets the user choose the output target without rerunning the optimizer.
