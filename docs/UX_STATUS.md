...
| Additional-track purpose | Working | The visible editor asks for the purpose first: no additions, improve difficult transitions, add spacing tracks as needed, or Extend playlist. Spacing-track repair and Extend playlist map to the native fixed-source extension request after calculating the requested final size. The dedicated destination-route mode is used only by Bliss me there; general playlist additions continue to use their own listener-facing purposes. |
...
| Add spacing tracks as needed | Working | Uses the source artists and albums to calculate the minimum repeat-safe final size, adds a bounded routeability buffer when the per-job maximum-additions budget allows it, and runs fixed-source membership selection. It does not use the bridge-trigger percentile or inspect individual gaps for optional transition bridges. |
