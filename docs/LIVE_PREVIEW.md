...
The initial activation restart was deferred while the Wohnzimmer player was active. Once every player was idle, LMS was stopped and started through piCorePlayer's service script. Live status then reported `ready=1`, `problem_count=0`, `ux_contract=extras-job-editor-v11`, and working mode `per-job-adaptive/optimize-or-preserve/none-auto-exact-seed-growth/create-copy`; the rendered Extras page contained the connected seed-growth controls.
