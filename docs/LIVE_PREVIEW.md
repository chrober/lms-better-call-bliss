...
## Version 0.11.0 fixed-source extension deployment
...
The initial activation restart was deferred while the Wohnzimmer player was active. Once every player was idle, LMS was stopped and started through piCorePlayer's service script. Live status then reported `ready=1`, `problem_count=0`, `ux_contract=extras-job-editor-v11`, and working mode `per-job-adaptive/optimize-or-preserve/none-auto-extend/create-copy`; the rendered Extras page contained the connected fixed-source extension controls.
...
The deployment validation deliberately stopped before **Create optimized copy**. The user subsequently invoked that explicit action. `Test Fixed-Source Extension Validation` was created and verified as LMS playlist ID `2515545` with 25 unique tracks. Its extended M3U contained 25 complete `#EXTURL`/`#EXTINF`/path triplets, and both original seed IDs occurred exactly once. The source playlist remained unchanged.

## Version 0.11.1 finalized fixed-source extension diagnostics
...
Read-only Extras job `preview-1785338076-0001` extended `Test` from two source tracks to exactly 25 tracks with 23 additions. It completed in 4,869 ms total and 4,690 ms native with a warm decoded-library cache. Fixed-source Adaptive relevance was 0.1190 best, 0.1633 mean, and 0.1835 furthest. The selected `adaptive-arc` route reported transition sum 3.4328, worst transition 0.3537, objective 4.1402, and arc error 3.1451.
